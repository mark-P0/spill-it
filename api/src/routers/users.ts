...
}
{
  ...
  UsersRouter[method](ep, async (req, res, next) => {
    ...
    if (username !== undefined) {
      logger.info("Checking username characters...");
      const isValidChars = isUsernameCharsValid(username);
      if (!isValidChars) {
        logger.error("New username characters invalid");
        return res.sendStatus(StatusCodes.BAD_REQUEST);
      }

      logger.info("Checking username existence...");
      const existingUserResult = await safeAsync(() =>
        readUserViaUsername(username),
      );
      if (!existingUserResult.success) {
        logger.error(formatError(existingUserResult.error));
        return res.sendStatus(StatusCodes.BAD_GATEWAY);
      }
